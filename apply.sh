...
if [ PRIMARY_MONGO -eq 1 ]; then
    puppet apply --logdest /root/primary-mongo.pp.log --verbose --debug --trace "${PUPPET_MANIFESTS}/primary-mongo.pp"
    sleep 120
    puppet apply --logdest /root/ceilometer.pp.log --verbose --debug --trace "${PUPPET_MANIFESTS}/ceilometer.pp"
else
    puppet apply --logdest /root/secondary-mongo.pp.log --verbose --debug --trace "${PUPPET_MANIFESTS}/secondary-mongo.pp"
fi
